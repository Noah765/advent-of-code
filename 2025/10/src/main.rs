...
fn first(input: String) -> u16 {
    parse_machines(&input)
        .map(|machine| {
            generate_button_subsets(&machine.buttons, machine.lights.len())
                .into_iter()
                .filter(|(_, x)| (0..x.len()).all(|i| (x[i] % 2 == 1) == machine.lights[i]))
                .map(|x| x.0)
                .min()
                .unwrap()
        })
...
fn second(input: String) -> u16 {
    parse_machines(&input)
        .map(|x| {
            calculate_minimum_button_presses(
                &x.joltages,
                &generate_button_subsets(&x.buttons, x.joltages.len()),
            )
            .unwrap()
        })
        .sum()
