...
fn first(input: String) -> usize {
    ...
    construct_sorted_box_pairs(&boxes)
        .into_iter()
        .take(connection_limit)
        .for_each(|(i, j)| update_circuits(i, j, &mut circuits));
...
fn construct_sorted_box_pairs(boxes: &[(u64, u64, u64)]) -> Vec<(usize, usize)> {
    ...
    pairs.sort_unstable_by_key(|&(i, j)| {
        boxes[i].0.abs_diff(boxes[j].0).pow(2)
            + boxes[i].1.abs_diff(boxes[j].1).pow(2)
            + boxes[i].2.abs_diff(boxes[j].2).pow(2)
    });
    pairs
}

fn update_circuits(
    first_box_index: usize,
    second_box_index: usize,
    circuits: &mut Vec<Vec<usize>>,
) {
    let first_circuit_index = circuits
        .iter()
        .position(|x| x.contains(&first_box_index))
        .unwrap();
    let second_circuit_index = circuits
        .iter()
        .position(|x| x.contains(&second_box_index))
        .unwrap();
    ...
    let first_circuit_index = if first_circuit_index < circuits.len() {
        first_circuit_index
    } else {
        second_circuit_index
    };
