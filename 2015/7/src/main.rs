...
fn second(input: String) -> u16 {
    ...
    let evaluable_gates = construct_circuit(&input);
    let gate_b = evaluable_gates
        .iter()
        .find(|x| x.borrow().identifier == "b")
        .unwrap();
    match gate_b.borrow_mut().inner {
        GateInner::Unary {
            gate_type: UnaryGateType::Noop,
            ref mut input,
        } => *input = Some(result),
        _ => panic!(),
    }
...
fn construct_circuit(input: &str) -> EvaluableGates {
    ...
    fn add_gate_output<'a>(
        gates: &Gates<'a>,
        gate_outputs: &mut GateOutputs<'a>,
        identifier: &'a str,
        output_type: GateOutputType,
        gate: Rc<RefCell<Gate<'a>>>,
    ) {
        if let Some(x) = gates.get(identifier) {
            x.borrow_mut()
                .outputs
                .push(GateOutput { output_type, gate });
        } else {
            gate_outputs
                .entry(identifier)
                .or_default()
    ...
    fn handle_unary_gate<'a>(
        gates: &mut Gates<'a>,
        evaluable_gates: &mut EvaluableGates<'a>,
        gate_outputs: &mut GateOutputs<'a>,
        input: &'a str,
        output: &'a str,
        gate_type: UnaryGateType,
        ...
        let gate = Rc::new(RefCell::new(Gate {
            identifier: output,
            inner: GateInner::Unary {
                gate_type,
                input: input.parse().ok(),
            },
            outputs: gate_outputs.remove(output).unwrap_or_default(),
        }));
    ...
    loop {
        ...
        for x in evaluable_gates.iter().map(|x| x.borrow()) {
            let value = match x.inner {
                ...
                GateInner::Unary {
                    gate_type: UnaryGateType::Not,
                    input,
                } => !input.unwrap(),
                GateInner::Binary {
                    gate_type: BinaryGateType::And,
                    lhs,
                    rhs,
                } => lhs.unwrap() & rhs.unwrap(),
                GateInner::Binary {
                    gate_type: BinaryGateType::Or,
                    lhs,
                    rhs,
                } => lhs.unwrap() | rhs.unwrap(),
                GateInner::Binary {
                    gate_type: BinaryGateType::Lshift,
                    lhs,
                    rhs,
                } => lhs.unwrap() << rhs.unwrap(),
                GateInner::Binary {
                    gate_type: BinaryGateType::Rshift,
                    lhs,
                    rhs,
                } => lhs.unwrap() >> rhs.unwrap(),
            };
            ...
            for output in &x.outputs {
                ...
                if let GateInner::Unary { input: Some(_), .. }
                | GateInner::Binary {
                    lhs: Some(_),
                    rhs: Some(_),
                    ..
                } = output.gate.borrow().inner
                {
