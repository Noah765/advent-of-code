...
fn first(input: String) -> u16 {
    ...
    distances
        .keys()
        .map(|start| visit(true, start, &mut HashSet::new(), &distances))
        .min()
        .unwrap()
}
...
fn second(input: String) -> u16 {
    ...
    distances
        .keys()
        .map(|start| visit(false, start, &mut HashSet::new(), &distances))
        .max()
        .unwrap()
}
...
fn visit<'a>(
    ...
    distances: &'a HashMap<&str, Vec<(&str, u16)>>,
) -> u16 {
    ...
    let iter = distances
        ...
        .filter_map(|(location, distance)| {
            if visited.contains(location) {
                ...
                Some(distance + visit(shortest, location, visited, distances))
            }
        });
    let result = if shortest {
...
fn parse_input(input: &str) -> HashMap<&str, Vec<(&str, u16)>> {
    ...
    for line in input.lines() {
        ...
        distances
            .entry(location1)
            .or_default()
            .push((location2, distance));
        distances
            .entry(location2)
            .or_default()
            .push((location1, distance));
    }
