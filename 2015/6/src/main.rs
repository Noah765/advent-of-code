...
fn first(input: String) -> usize {
    ...
    for instruction in parse_input(input) {
        ...
        grid.iter_mut()
            .take(instruction.right + 1)
            .skip(instruction.left)
            .for_each(|column| {
                column
                    .iter_mut()
                    .take(instruction.bottom + 1)
                    .skip(instruction.top)
                    .for_each(action)
            });
    }

    grid.into_iter()
        .map(|column| column.into_iter().filter(|x| *x).count())
        .sum()
}
...
fn second(input: String) -> u32 {
    ...
    grid.into_iter()
        .map(|column| column.into_iter().sum::<u32>())
        .sum()
}
...
fn parse_input(input: String) -> Vec<Instruction> {
    input
        ...
        .map(|x| {
            ...
            let first_coords: (usize, usize) = (
                first_coords.0.parse().unwrap(),
                first_coords.1.parse().unwrap(),
            );
            words_iter.next();
            let second_coords = words_iter.next().unwrap().split_once(',').unwrap();
            let second_coords = (
                second_coords.0.parse().unwrap(),
                second_coords.1.parse().unwrap(),
            );
